    ...
    switch($_GET["op"])
    {
        case 'guardaryeditar':
            if (empty($idingreso))
            {
                ...
                echo $rspta ?"Ingreso registrada" : "Ingreso no se pudo registrar";
            }
            else
            {
        ...
        case 'anular':
            ...
            echo $rspta ?"Ingreso Anular" : "Ingreso no se pudo Anular";
        break;
        ...
        while ($reg = $rspta->fetch_object())
                {
                    ...
                    $total=$total+($reg->precio_compra*$reg->cantidad); // La suma del Total  , Subtotal:precio_compra*$reg->cantidad
                }
        echo '<tfoot>
                                    <th>TOTAL</th>
                                    <th></th>
        ...
        break;
         ...
         case 'selectProveedor':
                ...
                while($reg=$rspta->fetch_object()) //Recorre toda la lista de personas que son proveedor y llena el select
                {
        ...
        while ($reg=$rspta->fetch_object()){
            $data[]=array(
                "0"=>'<button class="btn btn-warning"
                 onclick="agregarDetalle('.$reg->idarticulo.',\''.$reg->nombre.'\')">
